...
<div class="portlet box blue-chambray">
	...
	<form method="post" files = "true" class="form-horizontal" id="create-user-form" action="{{action('Admin\CategoryController@postCreateCategory')}}">
		{{csrf_field()}}
                ...
                <label class="col-md-3 control-label">Icon</label>
                    ...
                    <select class="form-control " name="icon">
                        ...
                        @foreach($icons as $icon)
                            <option id="{{$icon->id}}" class="icon" data-content="{{$icon->id}}" value="{{$icon->name}}" @if($icon->name == old('icon')) selected @endif >{{$icon->name}}</option>
                        @endforeach
                    </select>
                    </div>                   
                </div>
