	...
	<div class="row-fluid">
        <div class="span12">
            ...
            <div class="form-group">
                <div class="col-md-3">
                    <select class="form-control select-country" id="selectpicker" title="Country" name="country">
                        <option value="">Select Country</option>
                        @foreach($countries as $country)
                            <option id="{{$country->name}}" data-content = "{{$country->id}}">{{$country->name}}</option>
                        @endforeach
                    </select>
                ...
                <div class="table-responsive">
                    <table class="table table-bordered table-hover responsive-table">
                        ...
                        @foreach($search_results as $key => $result)
                            <tr>
                                <td>{{$key + 1}} <?php $map_url = "http://maps.google.com/maps?z=12&t=k&q=".$result->latitude.' '.$result->longtitude ?> <a target = "_blank" href="{{$map_url}}"> <i class="fa fa-map-marker" style="float:right"></i></a></td>
                                <td>{{$result->name}}</td>  
                                <td>{{$result->country->name}}</td>                                       
                                <td>
                                <a href="{{URL::to('/admin/show-city',$result->id)}}" type="button" class="btn btn-info">
                                    <i class="fa fa-book"></i> Show </a>
                                </td>
                                <td>
                                <a href = "{{URL::to('/admin/edit-city',$result->id)}}" type="button" class="btn btn-primary" >
                                    <i class="fa fa-edit"></i> Update </a>
                                </td>
                                <td>
                                <a type="button" data-toggle="modal" data-target = "#city_delete" href="#small" alt="{{$result->id}}" class="btn btn-danger show_modal_city">
