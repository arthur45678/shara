...
.directive('amMultiselect', ['$parse', '$document', '$compile', '$interpolate', '$filter', 'optionParser',
    ...
    return {
        ...
        function getHeaderText() {
            ...
            // {
            //     console.log(modelCtrl.$modelValue[i])
            // }
            var tagsDiv = document.getElementById('selected_languages_tags').innerHTML = "";
            
            angular.forEach(modelCtrl.$modelValue, function(model) {
                var tag = document.createElement('span');
                tag.setAttribute('class','language-tag');
                tag.addEventListener('click', uncheck);
                var tagText = document.createTextNode(model+' ×');
                tag.appendChild(tagText);
                var tagsDiv = document.getElementById('selected_languages_tags');
        ...
        function uncheck() {
            ...
            var res = inner.split(" ");
            var lang = res[0];
            console.log(inner,res,lang)
            for(var i = 0; i < scope.items.length; i++) {
               if(scope.items[i].label == lang)
               {
                   scope.select(scope.items[i]);
               }
            }
